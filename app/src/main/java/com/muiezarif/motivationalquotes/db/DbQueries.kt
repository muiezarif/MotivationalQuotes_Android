package com.muiezarif.motivationalquotes.db
...
class DbQueries(context: Context?) {
    ...
    fun getAllQuotes():ArrayList<QuotesModel>{
        ...
        try {
            cursor = mDb?.rawQuery("select * from motivational_quotes WHERE random() > 0000000222112865499", null)
            if (cursor != null) {
    ...
    fun getQuotesByAuthor(authorName:String):ArrayList<QuotesModel>{
        ...
        val operatorArray: ArrayList<QuotesModel> = ArrayList<QuotesModel>()
        try {
            cursor = mDb?.rawQuery("select * from motivational_quotes where author='"+authorName+"'", null)
            if (cursor != null) {
                val idCol = cursor.getColumnIndex("id")
                val authorNameCol = cursor.getColumnIndex("author")
                val favCol = cursor.getColumnIndex("fav")
    ...
    fun getAllAuthors():ArrayList<AuthorModel>{
        ...
        try {
            cursor = mDb?.rawQuery("select * from authors", null)
            if (cursor != null) {
                val idCol = cursor.getColumnIndex("id")
    ...
    fun updateQuoteFav(id:Int,check:Int){
        ...
        mDb?.update("motivational_quotes",values,"id=?",args)
        close()
    }
    ...
    fun getFavQuotes():ArrayList<QuotesModel>{
        ...
        try {
            cursor = mDb?.rawQuery("select * from motivational_quotes where fav="+1+"", null)
            if (cursor != null) {
                val idCol = cursor.getColumnIndex("id")
                val authorNameCol = cursor.getColumnIndex("author")
                val favCol = cursor.getColumnIndex("fav")
